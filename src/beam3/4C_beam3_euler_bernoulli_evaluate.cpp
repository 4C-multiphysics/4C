...
// assemble parts from tension
...
#else
  {
    ...
    // matrix for current positions and tangents
    Core::LinAlg::Matrix<nnode * dofpn, 1> disp_totlag(Core::LinAlg::Initialization::set_zero);
    ...
    Core::LinAlg::Matrix<15, 1, FAD> Res_inextensibility(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<15, 15, FAD> R_inextensibility(Core::LinAlg::Initialization::set_zero);
#endif
    ...
    Core::LinAlg::Matrix<3, 1, FAD> ortho_normal(Core::LinAlg::Initialization::set_zero);
    FAD rxrx_fad;
#endif
    ...
    R_tension_ANS.clear();
    double epsilon_ANS = 0.0;
    Core::LinAlg::Matrix<1, nnode * dofpn> lin_epsilon_ANS(Core::LinAlg::Initialization::set_zero);
    ...
    // Calculate epsilon at collocation points
#ifdef ANS_BEAM3EB
    Core::LinAlg::Matrix<3, 1> epsilon_cp(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, 3> tangent_cp(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, NODALDOFS * 6> lin_epsilon_cp(Core::LinAlg::Initialization::set_zero);

#ifdef BEAM3EBAUTOMATICDIFF
    Core::LinAlg::Matrix<3, 1, FAD> epsilon_cp_fad(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, 3, FAD> tangent_cp_fad(Core::LinAlg::Initialization::set_zero);
    ...
    Teuchos::ParameterList& params, Core::LinAlg::SerialDenseMatrix& elemat1)
{
  ...
  for (int node = 0; node < nnode; node++)
  {
    Core::LinAlg::Matrix<3, 1> t0(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, 1> t(Core::LinAlg::Initialization::set_zero);
    for (int i = 0; i < 3; i++)
    {
      t0(i) = t0_(i, node);
      t(i) = t_(i, node);
    ...
    Core::LinAlg::SerialDenseVector* force)        //!< element internal force vector
{
  ...
  Core::LinAlg::Matrix<ndim, 1> velbackground(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<ndim, ndim> velbackgroundgrad(Core::LinAlg::Initialization::set_zero);

  // evaluation point in physical space corresponding to a certain Gauss point in parameter space
  Core::LinAlg::Matrix<ndim, 1> evaluationpoint(Core::LinAlg::Initialization::set_zero);
  ...
  // Gauss point
  Core::LinAlg::Matrix<1, nnode * vpernode> N_i(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<1, nnode * vpernode> N_i_xi(Core::LinAlg::Initialization::set_zero);
    ...
    Core::LinAlg::SerialDenseVector* force)        //!< element internal force vector
{
  ...
  Core::LinAlg::Matrix<nnode * vpernode * ndim, 1> disp_totlag(
      Core::LinAlg::Initialization::set_zero);
    ...
    double& xi, const Core::LinAlg::Matrix<12, 1>& disp_totlag) const
{
  ...
  Core::LinAlg::Matrix<3, 1> r_s(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<1, 4> N_i_x(Core::LinAlg::Initialization::set_zero);
  const Core::FE::CellType distype = shape();
  ...
  Core::LinAlg::Matrix<1, 3> L_i(Core::LinAlg::Initialization::set_zero);
  Core::FE::shape_function_1d(L_i, xi, Core::FE::CellType::line3);
  double epsilon = 0.0;

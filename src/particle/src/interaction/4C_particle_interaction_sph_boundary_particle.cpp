...
void Particle::SPHBoundaryParticleAdami::init_boundary_particle_states(std::vector<double>& gravity)
{
  ...
  // get pointers to particle states
  ParticleContainerBundleStatePtrs& boundarypress =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::BoundaryPressure, boundarytypes_, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& boundaryvel =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::BoundaryVelocity, boundarytypes_, ParticleStatus::Owned);
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      if (sumj_wij_[static_cast<int>(type_i)][particle_i] > 0.0)
      {
        ...
        double* boundarypress_i =
            Particle::bundle_state_ptrs_index(boundarypress, type_i, status_i, particle_i);
        double* boundaryvel_i =
            Particle::bundle_state_ptrs_index(boundaryvel, type_i, status_i, particle_i, statedim);

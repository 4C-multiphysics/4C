...
void Particle::PDNeighborPairs::evaluate_particle_pairs()
{
  ...
  for (auto& potentialneighbors : particleengineinterface_->get_potential_particle_neighbors())
  {
    ...
    const double* pos_i = container_i->get_ptr_to_state(ParticleState::Position, particle_i);
    const double* pos_j = container_j->get_ptr_to_state(ParticleState::Position, particle_j);
    ...
    const double* rad_j = container_j->get_ptr_to_state(ParticleState::Radius, particle_j);

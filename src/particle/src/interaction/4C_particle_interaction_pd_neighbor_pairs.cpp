...
Particle::PDNeighborPairs::PDNeighborPairs(const MPI_Comm& comm)
    : map_hashkey_to_bondlist_entry_(std::make_unique<std::map<long, size_t>>()), comm_(comm)
{
  // empty constructor
}
...
void Particle::PDNeighborPairs::evaluate_particle_pairs()
{
  ...
  for (auto& potentialneighbors : particleengineinterface_->get_potential_particle_neighbors())
  {
    ...
    // DEM way
    if ((type_i != RigidPhase and type_i != BoundaryPhase) or
        (type_j != RigidPhase and type_j != BoundaryPhase))
      continue;

...
Particle::SPHPeridynamic::SPHPeridynamic(const Teuchos::ParameterList& params)
    : bondlist_(std::make_shared<std::vector<
      ...
      horizon_pd_(params.sublist("PD").get<double>("INTERACTION_HORIZON")),
      dx_pd_(params.sublist("PD").get<double>("PERIDYNAMIC_GRID_SPACING")),
      stiff_(params.sublist("SPH").get<double>("RIGIDPARTICLECONTACTSTIFF")),
      damp_(params.sublist("SPH").get<double>("RIGIDPARTICLECONTACTDAMP"))
{
  if (params.get<bool>("RIGID_BODY_MOTION"))
    FOUR_C_THROW("Peridynamic interaction is not available in combination with rigid body motion!");
}

void Particle::SPHPeridynamic::init(
    const std::shared_ptr<Particle::PDNeighborPairs> neighborpairs_solid_type)
{
  // init with particle phase relevant in peridynamic interaction
  ...
  neighborpairs_solid_type_ = neighborpairs_solid_type;
}
...
void Particle::SPHPeridynamic::init_peridynamic_bondlist()
{
      ...
      particleengineinterface_->get_potential_particle_neighbors())
  {
    ...
    // only rigid phase particles can undergo peridynamic interaction
...
void Particle::SPHPeridynamic::compute_interaction_forces() const
{
  ...
  // clear force of rigid phase particles
  Particle::ParticleContainer* container =
      particlecontainerbundle->get_specific_container(Particle::RigidPhase, Particle::Owned);
  ...
  while (iter < bondlist_->size())
  {
    ...
    if (stretch < critical_stretch)
    {
      ...
      double const fac = (12.00 * (young_i[0] + young_j[0]) * 0.5) /
                         (M_PI * horizon_pd_ * horizon_pd_ * horizon_pd_ * horizon_pd_) * stretch *
                         calculate_volume_correction_factor(xi_norm) * dx_pd_ * dx_pd_ * dx_pd_ *
  ...
  const DEMParticlePairData& dem_neighbor_pairs =
      neighborpairs_solid_type_->get_ref_to_particle_pair_data();
  for (const auto& particlepair : dem_neighbor_pairs)
  {
    // access values of local index tuples of particle i and j
    Particle::TypeEnum type_i;
    Particle::StatusEnum status_i;
...
void Particle::SPHPeridynamic::compute_acceleration() const
{
  ...
  Particle::ParticleContainer* container =
      particlecontainerbundle_->get_specific_container(Particle::RigidPhase, Particle::Owned);

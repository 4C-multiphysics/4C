...
{
  if (particle_params.get<bool>("RIGID_BODY_MOTION"))
  ...
  if (horizon_pd_ == 0.0)
    FOUR_C_THROW("Peridynamic INTERACTION_HORIZON must be greater than zero!");

  Teuchos::ParameterList binning_params = Global::Problem::instance()->binning_strategy_params();
  const double minimum_bin_size = binning_params.get<double>("BIN_SIZE_LOWER_BOUND");
  if (horizon_pd_ > minimum_bin_size)
    FOUR_C_THROW("Peridynamic INTERACTION_HORIZON must be smaller than BIN_SIZE_LOWER_BOUND!");
}
...
void Particle::SPHPeridynamic::compute_interaction_forces() const
{
  ...
  Core::IO::cout << "Number of dem_neighbor_pairs in peridynamic evaluation on this proc: "
                 << dem_neighbor_pairs.size() << Core::IO::endl;

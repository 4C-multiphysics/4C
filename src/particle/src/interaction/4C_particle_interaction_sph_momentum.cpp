...
/*---------------------------------------------------------------------------*
 | definitions                                                               |
 *---------------------------------------------------------------------------*/
Particle::SPHMomentum::SPHMomentum(const Teuchos::ParameterList& params)
    : params_sph_(params),
      ...
      purefluidtypes_({ParticleType::Phase1, ParticleType::Phase2}),
    ...
    std::map<ParticleType, std::set<ParticleState>>& particlestatestotypes) const
{
  ...
  for (auto& typeIt : particlestatestotypes)
  {
    ...
    if (transportvelocityformulation_ !=
      ...
      particlestates.insert({ParticleState::ModifiedVelocity, ParticleState::ModifiedAcceleration});
  }
}

void Particle::SPHMomentum::add_acceleration_contribution() const
{
  TEUCHOS_FUNC_TIME_MONITOR("Particle::SPHMomentum::add_acceleration_contribution");
...
void Particle::SPHMomentum::init_momentum_formulation_handler()
{
  ...
  switch (momentumformulationtype)
  {
    ...
    {
      momentumformulation_ = std::unique_ptr<Particle::SPHMomentumFormulationAdami>(
          new Particle::SPHMomentumFormulationAdami());
      break;
    }
    case Particle::MonaghanMomentumFormulation:
    {
      momentumformulation_ = std::unique_ptr<Particle::SPHMomentumFormulationMonaghan>(
          new Particle::SPHMomentumFormulationMonaghan());
      break;
    }
    default:
    {
...
void Particle::SPHMomentum::momentum_equation_particle_contribution() const
{
  ...
  ConstParticleContainerBundleStatePtrs& rad =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Radius, allfluidtypes_);
  ConstParticleContainerBundleStatePtrs& mass =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Mass, allfluidtypes_);
  ConstParticleContainerBundleStatePtrs& dens =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Density, allfluidtypes_);
  ConstParticleContainerBundleStatePtrs& press =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Pressure, allfluidtypes_);
  ConstParticleContainerBundleStatePtrs& vel =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Velocity, allfluidtypes_);
  ConstParticleContainerBundleStatePtrs& mod_vel = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::ModifiedVelocity, allfluidtypes_);
  ParticleContainerBundleStatePtrs& acc = particlecontainerbundle_->try_get_ptrs_to_state_writable(
      ParticleState::Acceleration, intfluidtypes_, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& mod_acc =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::ModifiedAcceleration, allfluidtypes_, ParticleStatus::Owned);
  ...
  for (const int particlepairindex : relindices)
  {
    ...
    const double* mod_vel_i = Particle::bundle_state_ptrs_index_or_nullptr(
        mod_vel, type_i, status_i, particle_i, statedim);
    double* acc_i = nullptr;
    if (intfluidtypes_.contains(type_i))
      acc_i =
          Particle::bundle_state_ptrs_index_or_nullptr(acc, type_i, status_i, particle_i, statedim);
    double* mod_acc_i = Particle::bundle_state_ptrs_index_or_nullptr(
        mod_acc, type_i, status_i, particle_i, statedim);
    ...
    const double* mod_vel_j = Particle::bundle_state_ptrs_index_or_nullptr(
        mod_vel, type_j, status_j, particle_j, statedim);
    double* acc_j = nullptr;
    if (intfluidtypes_.contains(type_j) and status_j == ParticleStatus::Owned)
      acc_j =
          Particle::bundle_state_ptrs_index_or_nullptr(acc, type_j, status_j, particle_j, statedim);
    double* mod_acc_j = nullptr;
    if (status_j == ParticleStatus::Owned)
      mod_acc_j = Particle::bundle_state_ptrs_index_or_nullptr(
          mod_acc, type_j, status_j, particle_j, statedim);
...
void Particle::SPHMomentum::momentum_equation_particle_boundary_contribution() const
{
  ...
  ParticleContainerBundleStatePtrs& mod_acc =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::ModifiedAcceleration, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& force =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::Force, ParticleStatus::Owned);
  ...
  for (const int particlepairindex : relindices)
  {
    ...
    if (status_i == ParticleStatus::Owned)
      mod_acc_i = Particle::bundle_state_ptrs_index_or_nullptr(
          mod_acc, type_i, status_i, particle_i, statedim);
    ...
    if (status_j == ParticleStatus::Owned)
      force_j = Particle::bundle_state_ptrs_index_or_nullptr(
          force, type_j, status_j, particle_j, statedim);
    ...
    if (acc_i) ParticleUtils::vec_add(acc_i, acc_ij);
    if (mod_acc_i) ParticleUtils::vec_add(mod_acc_i, mod_acc_ij);

    // add contribution to neighboring boundary particle j
    if (force_j) ParticleUtils::vec_add_scale(force_j, -mass_i[0], acc_ij);
  }
}
...
void Particle::SPHMomentum::momentum_equation_particle_wall_contribution() const
{
  ...
  ConstParticleContainerBundleStatePtrs& pos = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Position, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& rad = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Radius, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& mass = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Mass, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& dens = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Density, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& press = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Pressure, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& vel = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Velocity, intfluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& mod_vel = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::ModifiedVelocity, intfluidtypes_, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& acc = particlecontainerbundle_->try_get_ptrs_to_state_writable(
      ParticleState::Acceleration, intfluidtypes_, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& mod_acc =
      particlecontainerbundle_->try_get_ptrs_to_state_writable(
          ParticleState::ModifiedAcceleration, intfluidtypes_, ParticleStatus::Owned);
  ...
  for (const int particlewallpairindex : relindices)
  {
    ...
    ParticleUtils::vec_set(r_jl_weighted, weighteddistancevector[particlewallpairindex].data());
    ...
    const double inv_norm_dist_jl_weighted =
        1.0 / ParticleUtils::vec_dot(r_jl_weighted, particlewallpair.e_ij_);
        ...
        virtualwallparticle_->get_relative_positions_of_virtual_particles())
    {
      ...
      double r_jk[3];
      ParticleUtils::vec_set_scale(r_jk, virtualparticle[0], particlewallpair.e_ij_);
      ParticleUtils::vec_add_scale(r_jk, virtualparticle[1], t_j_1);
      ParticleUtils::vec_add_scale(r_jk, virtualparticle[2], t_j_2);

      // vector from virtual particle k to particle i
      double r_ik[3];
      ParticleUtils::vec_set(r_ik, r_ij);
      ParticleUtils::vec_add(r_ik, r_jk);
      ...
      if (absdist < rad_i[0])
      {
        ...
        ParticleUtils::vec_set(r_kl_weighted, r_jl_weighted);
        ...
        const double temp_press_k = weightedpressure[particlewallpairindex] +
                                    ParticleUtils::vec_dot(r_kl_weighted,
                                        weightedpressuregradient[particlewallpairindex].data());
        ...
        ParticleUtils::vec_set_scale(temp_vel_k, 1 + fac, vel_j);
        ParticleUtils::vec_add_scale(
            temp_vel_k, -fac, weightedvelocity[particlewallpairindex].data());
    ...
    ParticleUtils::vec_add(acc_i, sumk_acc_ik);
    if (mod_acc_i) ParticleUtils::vec_add(mod_acc_i, sumk_mod_acc_ik);
    ...
    if (writeinteractionoutput or walldatastate->get_force_col() != nullptr)
      ParticleUtils::vec_set_scale(wallcontactforce, -mass_i[0], sumk_acc_ik);
    ...
    if (writeinteractionoutput)
    {
      ...
      ParticleUtils::vec_sub(wallcontactpoint, r_ij);

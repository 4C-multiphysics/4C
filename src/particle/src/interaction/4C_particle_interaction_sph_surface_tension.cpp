    ...
    const std::shared_ptr<Particle::SPHNeighborPairs> neighborpairs)
{
  ...
  {
    std::vector<ParticleState> states{ParticleState::InterfaceNormal};
...
void Particle::SPHSurfaceTension::set_current_time(const double currenttime)
{
  time_ = currenttime;
}
    ...
    std::map<ParticleType, std::set<ParticleState>>& particlestatestotypes) const
{
  ...
  for (auto& typeIt : particlestatestotypes)
  {
    ...
    particlestates.insert({ParticleState::ColorfieldGradient, ParticleState::InterfaceNormal,
        ParticleState::Curvature});

    if (haveboundarytypes)
      particlestates.insert({ParticleState::WallColorfield, ParticleState::WallInterfaceNormal});
  }
}

void Particle::SPHSurfaceTension::compute_interface_quantities()
{
  TEUCHOS_FUNC_TIME_MONITOR("Particle::SPHSurfaceTension::compute_interface_quantities");
...
void Particle::SPHSurfaceTension::add_acceleration_contribution()
{
  TEUCHOS_FUNC_TIME_MONITOR("Particle::SPHSurfaceTension::add_acceleration_contribution");
...
void Particle::SPHSurfaceTension::compute_colorfield_gradient() const
{
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    container_i->clear_state(ParticleState::ColorfieldGradient);
  }
  ...
  ConstParticleContainerBundleStatePtrs& mass =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Mass, fluidtypes_);
  ConstParticleContainerBundleStatePtrs& dens =
      particlecontainerbundle_->try_get_ptrs_to_state(ParticleState::Density, fluidtypes_);
  ParticleContainerBundleStatePtrs& cfg = particlecontainerbundle_->try_get_ptrs_to_state_writable(
      ParticleState::ColorfieldGradient, fluidtypes_);
  ...
  ConstParticleContainerBundleStatePtrs& rad = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Radius, fluidtypes_, ParticleStatus::Owned);
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      if (not(cfg_i_norm > (1.0e-10 * rad_i[0]))) ParticleUtils::vec_clear(cfg_i);
    }
  }
}

void Particle::SPHSurfaceTension::compute_interface_normal() const
{
  // get pointers to particle states
  const int statedim = Particle::enum_to_state_dim(ParticleState::Position);
  ConstParticleContainerBundleStatePtrs& rad = particlecontainerbundle_->try_get_ptrs_to_state(
  ...
  ParticleContainerBundleStatePtrs& ifn = particlecontainerbundle_->try_get_ptrs_to_state_writable(
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      if (cfg_i_norm > (1.0e-10 * rad_i[0]))
        ParticleUtils::vec_set_scale(ifn_i, 1.0 / cfg_i_norm, cfg_i);
      else
        ParticleUtils::vec_clear(ifn_i);
    }
  }
}

void Particle::SPHSurfaceTension::compute_wall_colorfield_and_wall_interface_normal() const
{
  // iterate over fluid particle types
  for (const auto& type_i : fluidtypes_)
  {
    ...
    container_i->clear_state(ParticleState::WallColorfield);

    // clear wall interface normal state
    container_i->clear_state(ParticleState::WallInterfaceNormal);
  }
  ...
  for (const int particlepairindex : relindices)
  {
    ...
    if (fluidtypes_.contains(type_i))
    {
      ...
      const double fac = (ParticleUtils::pow<2>(V_i) + ParticleUtils::pow<2>(V_j)) * dens_i[0] /
                         (V_i * (dens_i[0] + material_j->initDensity_));

      // evaluate transition factor below reference temperature
      double tempfac = 1.0;
      if (trans_d_t_wet_ > 0.0)
        tempfac = ParticleUtils::comp_lin_trans(
            temp_j[0], trans_ref_temp_ - trans_d_t_wet_, trans_ref_temp_);

      // sum contribution of neighboring boundary particle j
      wallcf_i[0] += tempfac * fac * particlepair.Wij_;
      ParticleUtils::vec_add_scale(
          wallifn_i, tempfac * fac * particlepair.dWdrij_, particlepair.e_ij_);
    }

    // evaluate contribution of neighboring boundary particle i
    if (fluidtypes_.contains(type_j) and status_j == ParticleStatus::Owned)
    {
      // get material for current particle type
      ...
      const double fac = (ParticleUtils::pow<2>(V_i) + ParticleUtils::pow<2>(V_j)) * dens_j[0] /
      ...
      if (trans_d_t_wet_ > 0.0)
        tempfac = ParticleUtils::comp_lin_trans(
            temp_i[0], trans_ref_temp_ - trans_d_t_wet_, trans_ref_temp_);

      // sum contribution of neighboring boundary particle i
      wallcf_j[0] += tempfac * fac * particlepair.Wji_;
      ParticleUtils::vec_add_scale(
          wallifn_j, -tempfac * fac * particlepair.dWdrji_, particlepair.e_ij_);
    }
  }
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      if (wallifn_i_norm > (1.0e-10 * rad_i[0]))
        ParticleUtils::vec_set_scale(wallifn_i, 1.0 / wallifn_i_norm, wallifn_i);
      else
        ParticleUtils::vec_clear(wallifn_i);
    }
  }
}

void Particle::SPHSurfaceTension::correct_triple_point_normal() const
{
  // get pointers to particle states
  const int statedim = Particle::enum_to_state_dim(ParticleState::Position);
  ConstParticleContainerBundleStatePtrs& rad = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::Radius, fluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& wallifn = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::WallInterfaceNormal, fluidtypes_, ParticleStatus::Owned);
  ConstParticleContainerBundleStatePtrs& wallcf = particlecontainerbundle_->try_get_ptrs_to_state(
      ParticleState::WallColorfield, fluidtypes_, ParticleStatus::Owned);
  ParticleContainerBundleStatePtrs& ifn = particlecontainerbundle_->try_get_ptrs_to_state_writable(
      ParticleState::InterfaceNormal, fluidtypes_, ParticleStatus::Owned);
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      const double* wallcf_i =
          Particle::bundle_state_ptrs_index(wallcf, type_i, status_i, particle_i);
      double* ifn_i =
          Particle::bundle_state_ptrs_index(ifn, type_i, status_i, particle_i, statedim);
      ...
      double f_i = ParticleUtils::comp_lin_trans(wallcf_i[0], tpn_corr_cf_low_, tpn_corr_cf_up_);

      // determine wall interface tangential
      double wallift_i[3];
      ParticleUtils::vec_set(wallift_i, ifn_i);
      ParticleUtils::vec_add_scale(wallift_i, -ParticleUtils::vec_dot(ifn_i, wallifn_i), wallifn_i);

      // norm of wall interface tangential
      const double wallift_i_norm = ParticleUtils::vec_norm_two(wallift_i);

      // scale or clear wall interface tangential
      if (wallift_i_norm > (1.0e-10 * rad_i[0]))
        ParticleUtils::vec_set_scale(wallift_i, 1.0 / wallift_i_norm, wallift_i);
      else
        ParticleUtils::vec_clear(wallift_i);

      // determine triple point normal
      double tpn_i[3];
      ParticleUtils::vec_set_scale(tpn_i, std::sin(theta_0), wallift_i);
      ParticleUtils::vec_add_scale(tpn_i, -std::cos(theta_0), wallifn_i);

      // determine corrected interface normal
      double corifn_i[3];
      ParticleUtils::vec_set_scale(corifn_i, f_i, ifn_i);
      ParticleUtils::vec_add_scale(corifn_i, (1.0 - f_i), tpn_i);

      // norm of corrected interface normal
      const double corifn_i_norm = ParticleUtils::vec_norm_two(corifn_i);

      // scale or clear interface normal
      if (corifn_i_norm > (1.0e-10 * rad_i[0]))
        ParticleUtils::vec_set_scale(ifn_i, 1.0 / corifn_i_norm, corifn_i);
      else
        ParticleUtils::vec_clear(ifn_i);
    }
  }
...
void Particle::SPHSurfaceTension::compute_curvature() const
{
  ...
  for (const int particlepairindex : relindices)
  {
    ...
    if (not(ParticleUtils::vec_norm_two(ifn_i) > 0.0) or
        not(ParticleUtils::vec_norm_two(ifn_j) > 0.0))
      continue;
    ...
    double n_ij[3];
    ParticleUtils::vec_set(n_ij, ifn_i);
    ParticleUtils::vec_add_scale(n_ij, -signfac, ifn_j);

    const double nij_eij = ParticleUtils::vec_dot(n_ij, particlepair.e_ij_);
  ...
  // get pointer to particle states
  ParticleContainerBundleStatePtrs& curv = particlecontainerbundle_->try_get_ptrs_to_state_writable(
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      const double* ifn_i =
          Particle::bundle_state_ptrs_index(ifn, type_i, status_i, particle_i, statedim);
      double* curv_i = Particle::bundle_state_ptrs_index(curv, type_i, status_i, particle_i);
...
void Particle::SPHSurfaceTension::compute_surface_tension_contribution() const
{
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      if (trans_d_t_surf_ > 0.0)
        tempfac =
            ParticleUtils::lin_trans(temp_i[0], trans_ref_temp_, trans_ref_temp_ + trans_d_t_surf_);

      // add contribution to acceleration
      ParticleUtils::vec_add_scale(
          acc_i, -timefac * tempfac * alpha * curv_i[0] / dens_i[0], cfg_i);
    }
  }
}

void Particle::SPHSurfaceTension::compute_temp_grad_driven_contribution() const
{
  // evaluate surface tension time ramp function
  double timefac = 1.0;
  if (timerampfct_ > 0)
  ...
  for (const auto& type_i : fluidtypes_)
  {
    ...
    for (int particle_i = 0; particle_i < container_i->particles_stored(); ++particle_i)
    {
      ...
      ParticleUtils::vec_set(tempgrad_i_proj, tempgrad_i);
      ParticleUtils::vec_add_scale(
          tempgrad_i_proj, -ParticleUtils::vec_dot(tempgrad_i, ifn_i), ifn_i);

      // evaluate transition factor above reference temperature
      double tempfac = 1.0;
      if (trans_d_t_mara_ > 0.0)
        tempfac =
            ParticleUtils::lin_trans(temp_i[0], trans_ref_temp_, trans_ref_temp_ + trans_d_t_mara_);

      // add contribution to acceleration
      ParticleUtils::vec_add_scale(acc_i,
          timefac * tempfac * alpha_t_ * ParticleUtils::vec_norm_two(cfg_i) / dens_i[0],
          tempgrad_i_proj);
    }
  }
}

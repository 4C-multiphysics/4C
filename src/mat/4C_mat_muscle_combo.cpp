    ...
    const int eleGID)
{
  ...
  Core::LinAlg::Matrix<3, 3> invC(
      Core::LinAlg::Initialization::leave_uninitialized);  // matrix notation
  invC.invert(C);                                          // invC = C^-1
  Core::LinAlg::Matrix<6, 1> invCv(
      Core::LinAlg::Initialization::leave_uninitialized);        // Voigt notation
  ...
  // product invC*L
  Core::LinAlg::Matrix<3, 3> invCL(Core::LinAlg::Initialization::leave_uninitialized);
  ...
  Core::LinAlg::Matrix<6, 1> LomegaaMv(Core::LinAlg::Initialization::leave_uninitialized);
  ...
  LfacomegaaM.update((1.0 + omegaa * alpha * std::pow(lambdaM, 2)) / (alpha * std::pow(lambdaM, 2)),
      M, 1.0);  // + fac*M
  Core::LinAlg::Matrix<6, 1> LfacomegaaMv(Core::LinAlg::Initialization::leave_uninitialized);
  Core::LinAlg::Voigt::Stresses::matrix_to_vector(LfacomegaaM, LfacomegaaMv);

  Core::LinAlg::Matrix<3, 3> transpCLomegaaM(Core::LinAlg::Initialization::leave_uninitialized);
  transpCLomegaaM.multiply_tn(1.0, C, LomegaaM);  // C^T*(L+omegaa*M)
  Core::LinAlg::Matrix<6, 1> transpCLomegaaMv(Core::LinAlg::Initialization::leave_uninitialized);
  ...
  Core::LinAlg::Matrix<3, 3> stressM(Core::LinAlg::Initialization::leave_uninitialized);

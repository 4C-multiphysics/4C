    ...
    const Core::IO::InputParameterContainer& container)
{
  ...
  else
  {
    ...
    Core::LinAlg::Matrix<6, 5> red(Core::LinAlg::Initialization::set_zero);
    red(0, 0) = 1.;
    red(1, 1) = 1.;
    red(2, 0) = -1.;
    red(2, 1) = -1.;
    ...
    const Core::LinAlg::Matrix<3, 3>& defgrd, const int gp, const int eleGID, const double temp)
{
  ...
  Core::LinAlg::Matrix<6, 1> idv(Core::LinAlg::Initialization::set_zero);
    ...
    const Core::LinAlg::Matrix<6, 9>* d_cauchy_dFpi, Core::LinAlg::Matrix<6, 6>* d_cauchy_ddp)
{
  ...
  Core::LinAlg::Matrix<6, 6> pdev(Core::LinAlg::Initialization::set_zero);
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      if (i == j)
        pdev(i, j) = 2. / 3.;
  ...
  if (*active || dDpHeta > 0. || deltaDp->norm_inf() > 0.)
  {
    ...
    Core::LinAlg::Matrix<9, 6> dFpiDdeltaDp(Core::LinAlg::Initialization::set_zero);
    for (int A = 0; A < 3; A++)
      for (int a = 0; a < 3; a++)
        for (int b = 0; b < 3; b++)
          for (int i = 0; i < 6; i++)
    ...
    if (dNCPdT != nullptr)
    {
      if (dis_mode() == Inpar::TSI::Taylor_Quinney)
      {
        ...
        Core::LinAlg::Matrix<6, 1> dHpDeta(Core::LinAlg::Initialization::set_zero);
    ...
    const Core::LinAlg::Matrix<6, 9>* d_cauchy_dFpi, Core::LinAlg::Matrix<6, 9>* d_cauchy_ddp)
{
  ...
  Core::LinAlg::Matrix<6, 9> psymdev(Core::LinAlg::Initialization::set_zero);
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      if (i == j)
        psymdev(i, j) = +2. / 3.;
  ...
  // symmetric identity
  Core::LinAlg::Matrix<6, 9> psym(Core::LinAlg::Initialization::set_zero);
  ...
  if (*active || dDpHeta > 0. || deltaLp->norm_inf() > 0.)
  {
    ...
    // Derivative of inverse plastic deformation gradient
    Core::LinAlg::Matrix<9, 9> dFpiDdeltaLp(Core::LinAlg::Initialization::set_zero);
    for (int A = 0; A < 3; A++)
      for (int a = 0; a < 3; a++)
        for (int b = 0; b < 3; b++)
          for (int i = 0; i < 9; i++)

...
)
{
  ...
  Core::LinAlg::Matrix<3, 1> n_coord_old(Core::LinAlg::Initialization::set_zero);
  n_old->coordinates(&n_coord_old(0, 0));

  Core::LinAlg::Matrix<3, 1> n_coord_new(Core::LinAlg::Initialization::set_zero);
  n_new->coordinates(&n_coord_new(0, 0));

  // check if moving node (ALE case)
  Core::LinAlg::Matrix<3, 1> n_diff(Core::LinAlg::Initialization::set_zero);
...
)
{
  ...
  if (!successful_check)  // apply small artificial movement in normal direction(translation)
  {
    ...
    Core::LinAlg::Matrix<2, 1> xi_side(Core::LinAlg::Initialization::set_zero);
  ...
  Core::LinAlg::Matrix<3, 1> rst(
      Core::LinAlg::Initialization::set_zero);  // local coordinates w.r.t space time element (r,s,t
                                                // !!!)
    ...
    const Core::LinAlg::Matrix<3, numnode_space_time>& xyze_st)
{
  ...
  Core::LinAlg::Matrix<nsd, numnode_space_time> deriv(Core::LinAlg::Initialization::set_zero);

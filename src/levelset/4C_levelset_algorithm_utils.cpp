        ...
        if (actnode->owner() == myrank_)
        {
          ...
          for (int iele = 0; iele < actnode->num_element(); iele++)
          {
            ...
            // in case of further distypes, move the following block to a templated function
            {
              ...
              Core::LinAlg::Matrix<nsd, nen> evel(Core::LinAlg::Initialization::set_zero);
              ...
              Core::LinAlg::Matrix<nen, 1> funct(Core::LinAlg::Initialization::set_zero);
...
void ScaTra::LevelSetAlgorithm::manipulate_fluid_field_for_gfunc()
{
  ...
  for (int lnodeid = 0; lnodeid < discret_->num_my_row_nodes(); ++lnodeid)
  {
    ...
    Core::LinAlg::Matrix<3, 1> fluidvel(Core::LinAlg::Initialization::set_zero);
    ...
    if (foundit == allcollectednodes.end())
    {
      ...
      Core::LinAlg::Matrix<3, 2> closestnodedata(Core::LinAlg::Initialization::set_zero);
      {

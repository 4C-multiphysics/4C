    ...
    T>::integrate_local_evolution_equations_implicit(const T dt)
{
  ...
  const auto EvaluateLocalNewtonLinearSystem = [&]()
  {
    ...
    Core::LinAlg::Matrix<2, 1, T> residuum(Core::LinAlg::Initialization::leave_uninitialized);
    ...
    Core::LinAlg::Matrix<2, 2, T> drdx(Core::LinAlg::Initialization::leave_uninitialized);
  ...
  Core::LinAlg::Matrix<2, 1, T> x_np(Core::LinAlg::Initialization::leave_uninitialized);
  x_np(0) = states_.back().growth_scalar;
  x_np(1) = states_.back().lambda_r;

  Core::LinAlg::Matrix<2, 2, T> K(Core::LinAlg::Initialization::leave_uninitialized);
  Core::LinAlg::Matrix<2, 1, T> b(Core::LinAlg::Initialization::leave_uninitialized);
  ...
  Core::LinAlg::Matrix<2, 1, T> d_lambda_r_d_residuum(
      Core::LinAlg::Initialization::leave_uninitialized);

  ...
  switch (shape)
  {
    ...
    case Mortar::Element::bilindual2D:
    {
      ...
      if (mo_data().dual_shape() == nullptr)
      {
        ...
        Core::LinAlg::Matrix<nnodes, nnodes> me(Core::LinAlg::Initialization::set_zero);
        Core::LinAlg::Matrix<nnodes, nnodes> de(Core::LinAlg::Initialization::set_zero);
    ...
    case Mortar::Element::serendipitydual2D_only_lin:
    {
      ...
      if (mo_data().dual_shape() == nullptr)
      {
        ...
        // reduce me to non-zero nodes before inverting
        Core::LinAlg::Matrix<nnodeslin, nnodeslin> melin(Core::LinAlg::Initialization::set_zero);
        for (int j = 0; j < nnodeslin; ++j)
          for (int k = 0; k < nnodeslin; ++k) melin(j, k) = me(j, k);

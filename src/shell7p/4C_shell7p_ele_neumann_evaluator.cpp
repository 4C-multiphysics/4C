    ...
    Core::LinAlg::SerialDenseMatrix* element_stiffness_matrix, double total_time)
{
  ...
  for (int gp = 0; gp < intpoints.num_points(); ++gp)
  {
    ...
    Core::LinAlg::Matrix<num_dim, num_dim> g_metrics_kovariant(
        Core::LinAlg::Initialization::set_zero);

    Core::LinAlg::Matrix<numnod, num_dim> x_refe(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<numnod, num_dim> x_curr(Core::LinAlg::Initialization::set_zero);
    ...
    // -> g3 = (g1 x g2) / (|g1 x g2 |)
    Core::LinAlg::Matrix<num_dim, 1> g3(Core::LinAlg::Initialization::set_zero);

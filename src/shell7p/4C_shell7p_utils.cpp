    ...
    const Core::Elements::Element& ele, Core::LinAlg::SerialDenseMatrix& nodal_directors)
{
  ...
  for (int i = 0; i < num_node; ++i)
  {
    ...
    // -> a3 = (a1 x a2) / (|a1 x a2 |)
    Core::LinAlg::Matrix<num_dim, 1> a1a2crossprod(Core::LinAlg::Initialization::set_zero);
...
{
  Core::LinAlg::Matrix<3, 1> davn(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<3, 1> averdir(Core::LinAlg::Initialization::set_zero);
    ...
    std::map<int, std::vector<double>>& director_map)
{
  ...
  Core::LinAlg::Matrix<num_dim, max_ele> collaverdir(Core::LinAlg::Initialization::set_zero);
  ...
  for (const auto& act_node : dis.my_row_node_range())
  {
    ...
    {
      Core::LinAlg::Matrix<num_dim, 1> nodal_director(Core::LinAlg::Initialization::set_zero);

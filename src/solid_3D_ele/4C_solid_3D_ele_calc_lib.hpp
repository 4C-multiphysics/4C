...
namespace Discret::Elements
{
  ...
  {
    const std::array FdNdX = [&F, &jacobian_mapping]()
    {
      std::array<Core::LinAlg::Tensor<double, Core::FE::dim<celltype>>,
          Core::FE::num_nodes(celltype)>
          FdNdX;
      std::transform(jacobian_mapping.dNdX.begin(), jacobian_mapping.dNdX.end(), FdNdX.begin(),
          [&F](const auto& dNdX) { return F * dNdX; });
    ...
    for (std::size_t i = 0; i < Core::FE::num_nodes(celltype); ++i)
    {
      ...
      add_nodal_contribution<celltype>(i, i,
          Core::LinAlg::einsum_sym<"i", "ijkl", "l">(FdNdX[i], stress.cmat_, FdNdX[i]) *
              integration_fac,
          stiffness_matrix);
    ...
    for (std::size_t i = 0; i < Core::FE::num_nodes(celltype); ++i)
    {
      ...
      for (std::size_t j = i + 1; j < Core::FE::num_nodes(celltype); ++j)
      {
        add_nodal_contribution<celltype>(i, j,
            Core::LinAlg::einsum<"i", "ijkl", "l">(FdNdX[i], stress.cmat_, FdNdX[j]) *
                integration_fac,
            stiffness_matrix);

  ...
  switch (action)
  {
    case Core::Elements::struct_calc_nlnstiff:
    {
      ...
      if (la.size() > 1)
      {
        ...
        const SolidPoroPrimaryVariables primary_variables =
            extract_solid_poro_primary_variables<PorosityFormulation::explicit_porosity>(
        ...
        std::visit(
            [&](auto& interface)
            {
              interface->evaluate_nonlinear_force_stiffness(*this, this->struct_poro_material(),
                  ...
                  this->kinematic_type(), discretization, primary_variables, params, &force_displ,
                  &K_dd, &reaction_matrix);
            },
        ...
        std::visit(
            [&](auto& interface)
            {
              interface->evaluate_nonlinear_force_stiffness_porosity_od(*this,
                  this->struct_poro_material(), this->fluid_poro_material(),
                  this->get_anisotropic_permeability_property(), this->kinematic_type(),
                  discretization, primary_variables, *initial_porosity_, params, &force_p, &K_pp,
                  &K_dp, &K_pd);
            },
            solidporo_press_vel_based_calc_variant_);
        ...
        std::visit(
            [&](auto& interface)
            {
              interface->evaluate_nonlinear_force_stiffness_porosity_od(*this,
                  this->struct_poro_material(), this->fluid_poro_material(),
                  this->get_anisotropic_permeability_property(), this->kinematic_type(),
                  discretization, primary_variables, *initial_porosity_, params, &force_p, nullptr,
                  nullptr, nullptr);
            },
            solidporo_press_vel_based_calc_variant_);
    ...
    case Core::Elements::struct_poro_calc_fluidcoupling:
    {
      ...
      std::visit(
          [&](auto& interface)
          {
            interface->evaluate_nonlinear_force_stiffness_od(*this, this->struct_poro_material(),
                this->fluid_poro_material(), this->get_anisotropic_permeability_property(),
                this->kinematic_type(), discretization, primary_variables, params, &K_displ_fluid);
          },
          solidporo_press_vel_based_calc_variant_);
      ...
      std::visit(
          [&](auto& interface)
          {
            interface->evaluate_nonlinear_force_stiffness_od_porosity_od(*this,
                this->struct_poro_material(), this->fluid_poro_material(),
                this->get_anisotropic_permeability_property(), this->kinematic_type(),
                discretization, primary_variables, *initial_porosity_, params, &K_por_pres);
          },
          solidporo_press_vel_based_calc_variant_);

...
bool BeamInteraction::SUBMODELEVALUATOR::Crosslinking::post_partition_problem()
{
  ...
  for (unsigned int i = 0; i < numcolele; ++i)
  {
    ...
    for (auto const& iter : beamele_i->get_binding_spots())
    {
      ...
      Core::LinAlg::Matrix<3, 1> pos(Core::LinAlg::Initialization::set_zero);
      Core::LinAlg::Matrix<3, 3> triad(Core::LinAlg::Initialization::set_zero);
    ...
    std::vector<int>& newlinkermatid)
{
  ...
  Core::LinAlg::Matrix<3, 1> clpos(Core::LinAlg::Initialization::set_zero);
...
void BeamInteraction::SUBMODELEVALUATOR::Crosslinking::reset()
{
  ...
  for (auto const& iter : doublebondcl_)
  {
    ...
    // conditions
    // **************************** DEBUG ****************************************
    Core::LinAlg::Matrix<3, 1> dist(Core::LinAlg::Initialization::set_zero);
    ...
    Core::Nodes::Node* crosslinker_i, BeamInteraction::Data::CrosslinkerData* cldata_i)
{
  ...
  Core::LinAlg::Matrix<3, 1> newclpos(Core::LinAlg::Initialization::set_zero);
    ...
    Core::Nodes::Node* node_i) const
{
  ...
  for (auto const& sphere_iter : neighboring_col_spheres)
  {
    ...
    Core::LinAlg::Matrix<3, 1> sphere_pos(Core::LinAlg::Initialization::set_zero);
    ...
    bool checklinkingprop) const
{
  ...
  Core::LinAlg::Matrix<3, 1> occ_bindingspot_beam_tangent(Core::LinAlg::Initialization::set_zero);
  if (cldata_i->get_number_of_bonds() == 1)
  ...
  Core::LinAlg::Matrix<3, 1> curr_bindingspot_beam_tangent(Core::LinAlg::Initialization::set_zero);
  for (unsigned int idim = 0; idim < 3; ++idim)

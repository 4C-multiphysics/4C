    ...
    const int condition_coupling_id, const bool write_unique_ids)
{
  ...
  for (auto const& face_element_iterator : face_elements)
  {
    ...
    if (face_element_iterator.second->is_part_of_pair())
    {
      ...
      Core::LinAlg::Matrix<2, 1, double> xi(Core::LinAlg::Initialization::set_zero);
...
{
  Core::LinAlg::Matrix<3, 1, double> node_pos(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<3, 1, double> node_force(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<3, 1, double> node_moment(Core::LinAlg::Initialization::set_zero);

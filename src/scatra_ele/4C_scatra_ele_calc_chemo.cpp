    ...
    const Core::LinAlg::Matrix<nen_, 1>& sgconv, const Core::LinAlg::Matrix<nen_, 1>& diff)
{
  ...
  for (int condnum = 0; condnum < numcondchemo_; condnum++)
  {
    ...
    if (pair[k] == -1)  // if chemotractant
    {
      ...
      Core::LinAlg::Matrix<nen_, nen_> gradgradmatrix(Core::LinAlg::Initialization::set_zero);
      Core::LinAlg::Matrix<nen_, 1> bigterm(Core::LinAlg::Initialization::set_zero);
    ...
    const double scatrares, const double densnp)
{
  ...
  for (int condnum = 0; condnum < numcondchemo_; condnum++)
  {
    ...
    if (pair[k] == -1)  // if chemotractant
    {
      ...
      Core::LinAlg::Matrix<nen_, 1> gradfunctattr(Core::LinAlg::Initialization::set_zero);
  ...
  for (int condnum = 0; condnum < numcondchemo_; condnum++)
  {
    ...
    if (pair[k] == -1)
    {
      ...
      if (my::use2ndderiv_)
      {
        ...
        Core::LinAlg::Matrix<nen_, 1> laplace(Core::LinAlg::Initialization::set_zero);
        my::get_laplacian_strong_form(laplace);
        laplattractant = laplace.dot(my::ephinp_[partner]);
      }

      Core::LinAlg::Matrix<1, 1> chemoderivattr(Core::LinAlg::Initialization::set_zero);

    ...
    Core::LinAlg::SerialDenseVector& elevec3_epetra)
{
  ...
  switch (action)
  {
    ...
    case ScaTra::Action::calc_flux_domain:
    {
      ...
      Core::LinAlg::Matrix<3, nen_> eflux(Core::LinAlg::Initialization::set_zero);
    ...
    const Inpar::ScaTra::FluxType fluxtype, const int k)
{
  ...
  */
  ...
  for (int iquad = 0; iquad < intpoints.ip().nquad; ++iquad)
  {
    ...
    // get velocity at integration point
    Core::LinAlg::Matrix<nsd_, 1> velint(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<nsd_, 1> convelint(Core::LinAlg::Initialization::set_zero);
    ...
    const bool inverting, const bool calc_grad_phi)
{
  ...
  for (int iquad = 0; iquad < intpoints.ip().nquad; ++iquad)
  {
    ...
    for (int k = 0; k < numdofpernode_; k++)
    {
      ...
      Core::LinAlg::Matrix<nen_, 1> inv_ephinp(Core::LinAlg::Initialization::set_zero);
      if (inverting)
      {
    ...
    Core::LinAlg::SerialDenseVector& errors)
{
  ...
  switch (errortype)
  {
    case Inpar::ScaTra::calcerror_byfunction:
    {
      ...
      Core::LinAlg::Matrix<nsd_, 1> gradphi(Core::LinAlg::Initialization::set_zero);
      Core::LinAlg::Matrix<nsd_, 1> gradphi_exact(Core::LinAlg::Initialization::set_zero);
      Core::LinAlg::Matrix<nsd_, 1> deltagradphi(Core::LinAlg::Initialization::set_zero);

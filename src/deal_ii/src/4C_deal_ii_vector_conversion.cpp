...
namespace DealiiWrappers
{
      ...
      const Core::FE::Discretization& discretization, const Context<dim, spacedim>& context)
  {
    ...
    FOUR_C_ASSERT(dealii_to_four_c_map.IsOneToOne(), "Internal error.");
      ...
      const Core::FE::Discretization& discretization, const Epetra_Map& dealii_to_four_c_map)
  {
    ...
    Epetra_Import four_c_import(
        discretization.dof_row_map()->get_epetra_map(), dealii_to_four_c_map);
  ...
  VectorConverter<VectorType, dim, spacedim>::VectorConverter(
      ...
      : dealii_to_four_c_map_(create_dealii_to_four_c_map(dof_handler, discretization, context)),
        dealii_to_four_c_importer_(
            discretization.dof_row_map()->get_epetra_map(), dealii_to_four_c_map_),
  ...
  {
    FOUR_C_ASSERT_ALWAYS(four_c_vector.Map().PointSameAs(dealii_to_four_c_importer_.TargetMap()),
    ...
    vector_in_dealii_layout_.Export(four_c_vector, dealii_to_four_c_importer_, Insert);

    double* values_view = nullptr;
    vector_in_dealii_layout_.ExtractView(&values_view);
      ...
      Epetra_Vector& four_c_vector, const VectorType& dealii_vector) const
  {
    ...
    vector_in_dealii_layout_.ReplaceMyValues(

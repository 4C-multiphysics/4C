...
namespace DealiiWrappers
{
  ...
  /**
   * @brief Create an Epetra_Map object that maps from deal.II dofs to 4C gids.
   *
   * Given a dealii::DoFHandler @p dof_handler and Core::FE::Discretization @p discretization,
   * create and return an Epetra_Map that has the following properties:
   *
   ...
   *
   * This means, that the Epetra_Map object can be used to import and export vectors and matrices
   * between 4C and deal.II layout via an intermediate Epetra_Vector, which uses the returned
   * Epetra_Map. To go from a deal.II to a 4C vector, one first needs to copy the local elements
   * of the deal.II vector to the intermediate Epetra_Vector. Next, one can use the Epetra_Import
   * mechanism to communicate the intermediate vector to the 4C vector. This mechanism is
   * conveniently available through the VectorConverter class.
   *
   * The DoFHandler should be constructed on a dealii::Triangulation obtained from
   * CreateTriangulation(). As further input, this function requires the @p context that is returned
  ...
  class VectorConverter
  {
   ...
   public:
     ...
     */
    void to_four_c(Epetra_Vector& four_c_vector, const VectorType& dealii_vector) const;
     ...
     */
    void to_dealii(VectorType& dealii_vector, const Epetra_Vector& four_c_vector) const;
   ...
   private:
     ...
     * 4C.
     */
    Epetra_Map dealii_to_four_c_map_;
     ...
     */
    mutable Epetra_Vector vector_in_dealii_layout_;
  };

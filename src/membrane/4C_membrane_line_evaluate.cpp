    ...
    Core::LinAlg::SerialDenseMatrix* elemat1)
{
  ...
  */
  ...
  Core::LinAlg::Matrix<numnod_line_, noddof_> x(Core::LinAlg::Initialization::set_zero);
  ...
  Core::LinAlg::Matrix<numnod_line_, 1> shapefcts(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<1, numnod_line_> derivs(Core::LinAlg::Initialization::set_zero);
  ...
  for (int gp = 0; gp < intpointsline_.nquad; ++gp)
  {
    ...
    switch (ltype)
    {
      case neum_live:
      {
        ...
        // compute dXYZ / dr
        Core::LinAlg::Matrix<noddof_, 1> dxyzdr(Core::LinAlg::Initialization::set_zero);
        ...
        for (int i = 0; i < noddof_; ++i)
        {
          if (onoff[i])  // is this dof activated?
          {
            ...
            if (spa_func[i].has_value() && spa_func[i].value() > 0)
            {
              ...
              Core::LinAlg::Matrix<noddof_, 1> gp_coord(Core::LinAlg::Initialization::set_zero);

...
namespace ReducedLung
{
  void reduced_lung_main()
  {
    ...
    const Teuchos::ParameterList& redlung_params =
        Global::Problem::instance()->reduced_d_lung_params();

    const Teuchos::ParameterList& dyn_params = redlung_params.sublist("DYNAMICS");
    const Teuchos::ParameterList& tu_params =
        redlung_params.sublist("LUNG_TREE").sublist("TERMINAL_UNITS");
    const Teuchos::ParameterList& [[maybe_unused]] aw_params =
        redlung_params.sublist("LUNG_TREE")
            .sublist("AIRWAYS");  // TODO this will be used later when the airways are reimplemented

    const int linear_solver_number = dyn_params.get<int>("LINEAR_SOLVER");
    ...
    for (auto* ele : actdis->my_row_element_range())
    {
      ...
      else if (ele->element_type() == Discret::Elements::RedAcinusType::instance())
      {
        ...
        {
          IO::RheologicalModel rheological_model_name =
              tu_params.sublist("RHEOLOGICAL_MODEL")
                  .get<Core::IO::InputField<IO::RheologicalModel>>("RHEOLOGICAL_MODEL_TYPE")
                  .at(ele->id(), "RHEOLOGICAL_MODEL_TYPE");

          IO::ElasticityModel elasticity_model_name =
              tu_params.sublist("ELASTICITY_MODEL")
                  .get<Core::IO::InputField<IO::ElasticityModel>>("ELASTICITY_MODEL_TYPE")
                  .at(ele->id(), "ELASTICITY_MODEL_TYPE");

          if (rheological_model_name == IO::RheologicalModel::KelvinVoigt)
          {
            if (elasticity_model_name == IO::ElasticityModel::Linear)
            {
              add_terminal_unit_ele<KelvinVoigt, LinearElasticity>(
                  terminal_units, ele, local_element_id, tu_params);
            }
            else if (elasticity_model_name == IO::ElasticityModel::Ogden)
            {
              add_terminal_unit_ele<KelvinVoigt, OgdenHyperelasticity>(
                  terminal_units, ele, local_element_id, tu_params);
            }
          }
          else if (rheological_model_name == IO::RheologicalModel::FourElementMaxwell)
          {
            if (elasticity_model_name == IO::ElasticityModel::Linear)
            {
    ...
    */
    ...
    for (const auto* ele : actdis->my_row_element_range())
    {
      ...
      // the same way as assumed throughout this implementation! A top-down ordering of nodes
      // needs to be enforced during tree creation. This is to a large amount asserted during
      // creation of the coupling entities. However, special cases might slip through.
    ...
    const int results_every = dyn_params.get<int>("RESULTS_EVERY");
    // Time integration parameters.
    const double dt = dyn_params.get<double>("TIMESTEP");
    const int n_timesteps = dyn_params.get<int>("NUM_STEP");
    // Time loop
    if (Core::Communication::my_mpi_rank(comm) == 0)
    {

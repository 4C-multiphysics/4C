...
namespace FLD
{
  ...
  {
    Core::LinAlg::Matrix<9, iel> estrainrate_hat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<9, iel> ealphaij_hat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<1, iel> eexpression_hat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<1, iel> ealpha2_hat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, 3> strainrate_hat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<3, 3> alphaij_hat(Core::LinAlg::Initialization::set_zero);
    ...
    Core::LinAlg::Matrix<NSD, iel> deriv(Core::LinAlg::Initialization::set_zero);



    // get node coordinates of element
    Core::LinAlg::Matrix<NSD, iel> xyze(Core::LinAlg::Initialization::set_zero);
    for (int inode = 0; inode < iel; inode++)
    {
      xyze(0, inode) = ele->nodes()[inode]->x()[0];
      xyze(1, inode) = ele->nodes()[inode]->x()[1];
  ...
  */
      ...
      std::vector<double>& fsvel)
  {
    ...
    else  // no, so we calculate N from Re
    {
      ...
      switch (reflength)
      {
        case Inpar::FLUID::streamlength:
        {
          ...
          Core::LinAlg::Matrix<nsd, 1> velino(Core::LinAlg::Initialization::set_zero);
          if (vel_norm >= 1e-6)
    ...
    Core::LinAlg::Matrix<nsd, 1> B(Core::LinAlg::Initialization::set_zero);
    //                                  1
    //          |       1              |2
    //  kappa = | -------------------- |
    //          |  1 - alpha ^ (-4/3)  |
                ...
                Inpar::FLUID::FineSubgridVisc::no_fssgv) != Inpar::FLUID::FineSubgridVisc::no_fssgv)
    {
      ...
      //          +-                                 -+
      //
      Core::LinAlg::Matrix<nsd, nsd> velderxy(Core::LinAlg::Initialization::set_zero);
      velintderxy.multiply_nt(evel, derxy);
  ...
  */
    if (ele->is_ale())
    {
      // --------------------------------------------------
      // create matrix objects for nodal values
      Core::LinAlg::Matrix<nsd, iel> edispnp(Core::LinAlg::Initialization::set_zero);
  ...
  */
    ...
    for (int iquad = 0; iquad < intpoints.ip().nquad; ++iquad)
    {
      ...
      Core::LinAlg::Matrix<nsd, 1> xsi(Core::LinAlg::Initialization::set_zero);

  ...
  // Special treatment if we have any box face conditions
  {
    for (const auto& l : box_face_conditions)
    {
...

      {
        ...
        Core::Communication::broadcast(bottom_corner.data(), 3, 0, input.get_comm());
        Core::Communication::broadcast(top_corner.data(), 3, 0, input.get_comm());
        Core::Communication::broadcast(rotation_angle.data(), 3, 0, input.get_comm());
      }
      ...
      for (const auto* node : actdis.my_row_node_range())
      {
        ...
        for (int rotaxis = 2; rotaxis > -1; --rotaxis)
        {
          if (rotation_angle[rotaxis] != 0.0)
          {
            ...
            double calpha = cos(-rotation_angle[rotaxis] * M_PI / 180);
            double salpha = sin(-rotation_angle[rotaxis] * M_PI / 180);

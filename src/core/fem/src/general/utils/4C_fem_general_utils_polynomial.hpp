    ...
    */
    ...
    [[nodiscard]] double evaluate_derivative(const double point, const int deriv_order) const
    {
      switch (deriv_order)
      {
        case 1:
        {
          Core::LinAlg::Matrix<2, 1> derivatives(Core::LinAlg::Initialization::leave_uninitialized);
          evaluate(point, derivatives);
          return derivatives(1, 0);
        }
        case 2:
        {
          Core::LinAlg::Matrix<3, 1> derivatives(Core::LinAlg::Initialization::leave_uninitialized);
          evaluate(point, derivatives);
          return derivatives(2, 0);
        }
        case 3:
        {
          Core::LinAlg::Matrix<4, 1> derivatives(Core::LinAlg::Initialization::leave_uninitialized);
          evaluate(point, derivatives);

...
{
  Core::LinAlg::Matrix<3, 2> XAABB(Core::LinAlg::Initialization::set_zero);
    ...
    Core::LinAlg::Matrix<3, 1>& minDistCoords)
{
  ...
  Core::LinAlg::Matrix<3, 1> x_surface(Core::LinAlg::Initialization::set_zero);
  std::map<int, std::set<int>> nodeList;
    ...
    double& distance)
{
  bool pointFound = false;
  double min_distance = std::numeric_limits<double>::max();
  Core::LinAlg::Matrix<3, 1> distance_vector(Core::LinAlg::Initialization::set_zero);
  Core::LinAlg::Matrix<2, 1> elecoord(
      Core::LinAlg::Initialization::set_zero);  // starting value at element center
  ...
  if (eleGeoType == Core::Geo::HIGHERORDER)
  {
    ...
    for (int i = 0; i < surfaceElement->num_node(); i++)
    {
      ...
      {
        Core::LinAlg::Matrix<3, 1> physcoord(Core::LinAlg::Initialization::set_zero);
        Core::Geo::element_to_current_coordinates(

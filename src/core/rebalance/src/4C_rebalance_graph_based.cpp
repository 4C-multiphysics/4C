    ...
    Teuchos::ParameterList rebalanceParams, const Core::LinAlg::MultiVector<double>& initialWeights)
{
  ...
  std::vector<int> stride(initialWeights.num_vectors());
  for (int stride_num = 0; stride_num < initialWeights.num_vectors(); stride_num++)
    stride[stride_num] = initialWeights.num_vectors();

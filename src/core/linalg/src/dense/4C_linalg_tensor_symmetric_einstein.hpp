...
namespace Core::LinAlg
{
   ...
   *
   * @note This function assumes that the resulting tensor is symmetric!
   *
   ...
   *
   * @code{.cpp}
   * auto tensor_result = einsum_sym(einidx<0, 1>(F), einidx<0, 2>(C), einidx<2, 3>(F)); // F^T C F
   * @endcode
   *
  ...
  auto einsum_sym(const TensorIdx&... t)
  {
    ...
    static_assert(Internal::get_invalid_indexes<all_indexes>().size() == 0,
        "Invalid indexes in symmetric Einstein summation. Each index must be used at most twice.");
    static_assert(std::ranges::max(all_indexes) ==
                      Internal::make_unique_by<all_indexes, all_indexes>().size() - 1,
        "It is not allowed to leave out indexes in symmetric Einstein summation.");
    ...
    {
      Internal::SymmetricTensorTypeFromArray<value_type, dyadic_index_sizes> tensor_result{};
      Internal::EinsteinSummationItemTypeFromArray<decltype(tensor_result), dyadic_indexes> result{
          tensor_result};

      Internal::ConstExprSymmetricMultiForMaker<index_sizes, dyadic_indexes>::multi_for(
          [&](auto... i)
          {

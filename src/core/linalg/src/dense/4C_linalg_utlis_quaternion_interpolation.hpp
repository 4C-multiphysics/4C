...
namespace Core
{
  namespace LinAlg
  {
     ...
     *
     * This function calculates the cross product of vectors \p u and \p v,
     * and stores the result in \p result. All vectors are represented as
     * 3x1 matrices.
     *
     * @param u The first input vector (3x1 matrix).
     * @param v The second input vector (3x1 matrix).
     * @param result Reference to a 3x1 matrix where the cross product result will be stored.
     */
    void vector_cross_product(const Core::LinAlg::Matrix<3, 1>& u,
        const Core::LinAlg::Matrix<3, 1>& v, Core::LinAlg::Matrix<3, 1>& result);

    /**
     * @brief Normalizes the given vector to have unit length.
     *
     * This function modifies the input vector in place so that its Euclidean norm becomes 1.
     *
     * @tparam loc_dim The dimension of the vector.
     * @param vector Reference to the vector to be normalized. The vector is represented as a column
     * matrix.
     */
    template <unsigned int loc_dim>
    void normalize_vector(Core::LinAlg::Matrix<loc_dim, 1>& vector);

    /**
     * @namespace QuaternionInterpolation
     * @brief Provides functions and classes for quaternion interpolation, including spherical
     * linear interpolation (SLERP) and generalized SLERP for multiple quaternions.
     *
     * This namespace contains utilities for performing interpolation between quaternions, which are
     * commonly used to represent rotations in 3D space. The provided functionality includes:
     * - Spherical linear interpolation (SLERP) between two unit quaternions.
     * - A generalized SLERP class that supports interpolation among multiple quaternions, with
     * flexible weighting and reference location mechanisms.
     *
     * The main components are:
     * - `spherical_linear_interpolation`: Computes the SLERP between two unit quaternions for
     * a given interpolation parameter.
     * - `GeneralizedSphericalLinearInterpolator`: A template class that enables generalized
     * quaternion interpolation based on reference locations and custom weighting functions,
     * supporting both location-based and explicit weight-based interpolation.
     * The algorithm is based on the algorithm by Buss and Fillmore (2001).
     */
    namespace QuaternionInterpolation
    {
      /**
      ...
      class GeneralizedSphericalLinearInterpolator
      {
       ...
       private:
         ...
         *
         * @tparam loc_dim The local dimension parameter for the interpolator.
         ...
         * quaternion_2.
         */
        Core::LinAlg::Matrix<4, 1> project_quaternion(

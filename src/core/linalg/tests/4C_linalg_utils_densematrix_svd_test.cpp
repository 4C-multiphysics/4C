...
namespace
{
  ...
  {
    Core::LinAlg::Matrix<n, n> MHM(Core::LinAlg::Initialization::leave_uninitialized);
    MHM.multiply_tn(M, M);
      ...
      const Core::LinAlg::Matrix<cols, cols>& VT, const std::array<double, length>& singularValues)
  {
    ...
    Core::LinAlg::Matrix<rows, cols> QS(Core::LinAlg::Initialization::leave_uninitialized);
    Core::LinAlg::Matrix<rows, cols> A_result(Core::LinAlg::Initialization::leave_uninitialized);
    QS.multiply_nn(Q, S);

...
namespace
{
  ...
  TEST_F(GeometryPairLineToSurfaceTest, TestPointToSurfaceProjectionNormalInterpolationTri3)
  {
    ...
    // Point to project to.
    Core::LinAlg::Matrix<3, 1, double> point(Core::LinAlg::Initialization::set_zero);
    point(0) = 0.3;
    point(1) = 0.1;
    point(2) = 0.2;

    // Project the point to the surface.
    Core::LinAlg::Matrix<3, 1, double> xi(Core::LinAlg::Initialization::set_zero);
    ProjectionResult projection_result;
    pair.project_point_to_other(point, element_data_solid, xi, projection_result);

    // Check the results.
    Core::LinAlg::Matrix<3, 1, double> xi_result(Core::LinAlg::Initialization::set_zero);
  ...
  TEST_F(GeometryPairLineToSurfaceTest, TestLineToSurfaceIntersectionNormalInterpolationTri3)
  {
    ...
    Core::LinAlg::Matrix<3, 1, double> xi_start(Core::LinAlg::Initialization::set_zero);

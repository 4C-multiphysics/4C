...
namespace
{
  ...
  /// Birzle/Wall median FSLS parameters for viable lung parenchyma, used throughout as a
  /// physically meaningful default parameter set.
  ...
  /// demonstrate that the production formula is measurably more accurate for large k.
  ...
  TEST(FslsL1KernelTest, ConsistentTangentMatchesCentralFiniteDifference)
  {
    ...
    FslsKernelInput baseline_input;
    baseline_input.dt = dt;
    baseline_input.gp = 0;
    baseline_input.tau = kTau;
    baseline_input.alpha = kAlpha;
    baseline_input.beta = kBeta;
    baseline_input.solve_kind = FslsSolveKind::l1;
    baseline_input.previous_history = &history;
    baseline_input.l1 = {&cache, &step_cache};
    FslsStressVector q_current(Core::LinAlg::Initialization::zero);

...
namespace
{
  ...
  TEST_F(DruckerPragerTest, TestPackUnpack)
  {
    ...
    Core::LinAlg::Matrix<6, 1> ref_stress(Core::LinAlg::Initialization::set_zero);
    for (int i = 0; i < 3; ++i)
      ref_stress(i) =
  ...
  TEST_F(DruckerPragerTest, TestEvaluateHistory)
  {
    ...
    Teuchos::ParameterList paras;
    Core::LinAlg::Matrix<3, 3> defgrad(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<6, 1, FAD> ref_stress(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<6, 6> result_cmat(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<6, 1, FAD> result_stress(Core::LinAlg::Initialization::set_zero);
    druckprag_->evaluate(&defgrad, &input_strain, paras, &result_stress, &result_cmat, 0, 0);
    Core::LinAlg::Matrix<6, 6> ref_cmat(Core::LinAlg::Initialization::set_zero);
    for (int i = 0; i < 6; i++)
    {
      for (int j = 0; j < 6; j++)
      {

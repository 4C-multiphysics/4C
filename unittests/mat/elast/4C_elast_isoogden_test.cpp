...
namespace
{
  ...
  TEST_F(IsoOgdenTest, add_coefficients_stretches_modified)
  {
    ...
    Core::LinAlg::Matrix<3, 1> modstr(Core::LinAlg::Initialization::set_zero);
    modstr(0) = 0.9;
    modstr(1) = 0.8;
    modstr(2) = 0.7;

    // initialize resulting coefficients
    Core::LinAlg::Matrix<3, 1> modgamma(Core::LinAlg::Initialization::set_zero);
    Core::LinAlg::Matrix<6, 1> moddelta(Core::LinAlg::Initialization::set_zero);
